<script lang="ts">
    import { Upload, X, FileArchive, CheckCircle, AlertCircle } from '@lucide/svelte';
    ...
    async function uploadTheme() {
        ...
        try {
            ...
            xhr.addEventListener('load', () => {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    if (response.success) {
                        uploadState = 'success';
                        toast.success(
                            `테마 "${response.manifest.name}"가 성공적으로 업로드되었습니다.`
                        );

                        // 3초 후 다이얼로그 닫기 및 콜백 호출
                        setTimeout(() => {
                            dialogOpen = false;
                            removeFile();
                            onUploadSuccess();
            ...
            // 업로드 시작
            xhr.open('POST', 'http://localhost:5173/api/themes/upload');
            xhr.send(formData);
        } catch (error) {
            uploadState = 'error';
...
<Dialog bind:open={dialogOpen}>
    <DialogContent class="sm:max-w-md">
        <DialogHeader>
            <DialogTitle>테마 업로드</DialogTitle>
            <DialogDescription>ZIP 형식의 테마 파일을 업로드하세요. (최대 10MB)</DialogDescription>
        </DialogHeader>
