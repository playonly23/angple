...
import {
    ...
    savePromotionRules,
    type PromotionRule
} from '$lib/server/auth/auto-promotion.js';
...
export const PUT: RequestHandler = async ({ locals, request }) => {
    ...
    try {
        ...
        if (!Array.isArray(rules)) {
            return json(
                { error: { code: 'INVALID', message: 'rules는 배열이어야 합니다.' } },
                { status: 400 }
            );
        }
        ...
        for (const rule of rules) {
            if (
                typeof rule.fromLevel !== 'number' ||
                typeof rule.toLevel !== 'number' ||
                typeof rule.minLoginDays !== 'number' ||
                typeof rule.minXP !== 'number'
            ) {
                return json(
                    { error: { code: 'INVALID', message: '잘못된 규칙 형식입니다.' } },
                    { status: 400 }
                );
            }
            if (rule.fromLevel >= rule.toLevel) {
                return json(
                    { error: { code: 'INVALID', message: 'toLevel은 fromLevel보다 커야 합니다.' } },
                    { status: 400 }
                );
            }
            if (rule.minLoginDays < 0 || rule.minXP < 0) {
                return json(
                    { error: { code: 'INVALID', message: '조건 값은 0 이상이어야 합니다.' } },
                    { status: 400 }
                );
            }
        }
