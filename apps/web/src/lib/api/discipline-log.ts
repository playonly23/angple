 ...
 */
export function getPenaltyDisplay(period: number): { text: string; color: 'magenta' | 'orange' | 'red' } {
	if (period === -1) {
 ...
 */
export async function getDisciplineLogs(page = 1, limit = 20): Promise<DisciplineLogListResponse> {
	const url = `${API_BASE}/discipline-logs?page=${page}&limit=${limit}`;
...
export async function createDisciplineLog(req: CreateDisciplineLogRequest): Promise<DisciplineLogDetailResponse> {
