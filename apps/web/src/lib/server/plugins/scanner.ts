...
function isValidPluginDirectory(pluginPath: string): boolean {
    ...
    // plugin.json 파일이 있어야 함
    const manifestPath = join(pluginPath, 'plugin.json'); // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    return existsSync(manifestPath);
}

/**
 * plugin.json 파일을 읽고 검증
 * @param pluginDir 플러그인 디렉터리명 (안전한 이름, readdir에서 반환)
 * @param baseDir 플러그인이 위치한 기본 디렉터리 (PLUGINS_DIR 또는 CUSTOM_PLUGINS_DIR)
 */
function loadPluginManifest(pluginDir: string, baseDir: string): PluginManifest | null {
    // pluginDir는 readdir()에서 온 안전한 디렉터리명
    // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    const manifestPath = join(baseDir, pluginDir, 'plugin.json');
    ...
    try {
        ...
        if (!result.success) {
            console.error('❌ [Plugin Scanner] 플러그인 검증 실패:', { pluginDir });
 ...
 */
function scanDirectory(baseDir: string, plugins: Map<string, PluginManifest>): number {
 ...
 *
 * @returns 플러그인 ID를 key로 하는 매니페스트 맵
 */
