        ...
        <CardContent class="space-y-4">
            ...
            {:else}
                ...
                <div class="text-muted-foreground text-xs">
                    {periodLabel} · {direction === 'received'
                        ? '이 회원의 글에 반응한'
                        : '이 회원이 반응한'}
                                ...
                                <div class="bg-muted h-5 flex-1 overflow-hidden rounded-full">
                                    <div
                                        ...
                                        style="width: {Math.max(
                                            (entry.total_count / maxCount) * 100,
                                            3
                                        )}%"
                                    ></div>
                                </div>
                ...
                {#if (authStore.user?.mb_level ?? 0) >= 10 && entries.length >= 2}
                    ...
                    {#if topRatio > 0.3}
                        <div
                            class="mt-3 rounded-lg border border-yellow-300 bg-yellow-50 p-3 text-sm dark:border-yellow-700 dark:bg-yellow-900/20"
                        >
                            <span class="font-medium text-yellow-800 dark:text-yellow-200">
                                집중도 주의:
                            </span>
                            <span class="text-yellow-700 dark:text-yellow-300">
                                1위 {entries[0].mb_name}의 비중이 {(topRatio * 100).toFixed(0)}%로
                                높습니다. 상호 관계를 확인해보세요.
                            </span>
                        </div>
                    {/if}
